...
class ScheduleBaseViewController: BaseViewController, APIFetchDelegate, UITableViewDataSource, UITableViewDelegate {
    ...
    func today() -> Int {
        var day =  Calendar(identifier: .gregorian).component(.weekday, from: Date())
        // convert from Apple's format (Sunday starting) to our format (Monday starting)
        day -= 2
        if (day < 0) {
            day += 7
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if var cell = cell as? ScheduleShowCellDelegate {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if let showCell = cell as? ScheduleShowCellDelegate {
